const STORAGE_KEY = "dziennik_kierowcy_km_kraj_v5";
...
function getEntryFromForm() {
  ...
  const countryKm = endOdometer - startOdometer;
  ...
  if (endOdometer < startOdometer) {
    alert("Licznik końcowy nie może być mniejszy od licznika początkowego.");
    return null;
  }
...
function renderEntries() {
  const entries = sortEntriesByDateDesc(state.entries);
  ...
  els.entriesTableBody.innerHTML = entries.map((entry) => {
    const realIndex = state.entries.findIndex((item) => item === entry);
      ...
      <tr>
        ...
        <td>
          <div class="row-actions">
            <button type="button" class="btn btn-secondary btn-sm" data-action="edit" data-index="${realIndex}">Edytuj</button>
            <button type="button" class="btn btn-danger btn-sm" data-action="delete" data-index="${realIndex}">Usuń</button>
          </div>
        </td>
      </tr>
    `;
...
function exportCsv() {
  ...
  sortEntriesByDateDesc(state.entries).forEach((entry) => {
    lines.push([
      entry.date,
      entry.startPlace,
      entry.endPlace,
  ...
  a.href = url;
  a.download = "dziennik-kierowcy-km-po-kraju.csv";
...
function exportPdf() {
  ...
  const body = sortEntriesByDateDesc(state.entries).map((entry) => [
    entry.date,
    entry.startPlace,
    entry.endPlace,
...
init();  exportCsvBtn: document.getElementById("exportCsvBtn"),
  exportPdfBtn: document.getElementById("exportPdfBtn"),
  clearAllBtn: document.getElementById("clearAllBtn"),
};
